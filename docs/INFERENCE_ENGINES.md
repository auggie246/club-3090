# Inference engine comparison — vLLM, llama.cpp, SGLang, ktransformers

Pragmatic comparison of the four engines this stack interacts with most. **Versions captured 2026-05-07** — all four projects move fast; verify against upstream release notes before making a long-horizon decision. Linked sources at the bottom.
...
All four are actively developed. ktransformers is positioned as research but production paths exist (kt-kernel + SGLang).

---

## Hardware support

| Feature | vLLM | llama.cpp | SGLang | ktransformers |
|---|---|---|---|---|
...
| **Apple Silicon (Metal)** | ❌ | ✅ first-class | ✅ MLX backend (v0.5.10) | ❌ |
...
| Format | vLLM | llama.cpp | SGLang | ktransformers |
|---|---|---|---|---|
| **GGUF (Q*, K*, IQ*)** | ⚠️ Limited (recent UD-IQ1_S support) | ✅ Native | ⚠️ Recent | ⚠️ |
| **AutoRound INT4 (Marlin)** | ✅ + our [PR #40361](https://github.com/vllm-project/vllm/pull/40361) | ❌ | ✅ | ⚠️ |
| **GPTQ INT4** | ✅ | ❌ | ✅ | ✅ |
| **AWQ INT4** | ✅ | ❌ | ✅ | ⚠️ |
| **FP8 (e4m3, e5m2)** | ✅ multi-backend (Marlin/TRTLLM/MXFP8) | ⚠️ Limited | ✅ FlashInfer MXFP8 | ✅ Native (M2.x) |
| **FP4 / MXFP4** | ✅ (online MoE quant) | ⚠️ | ✅ MXFP4 kernels | ✅ DeepSeek-V4 / kt-kernel |
...
| KV format | vLLM | llama.cpp | SGLang | ktransformers |
|---|---|---|---|---|
| **FP16 / BF16** | ✅ | ✅ | ✅ | ✅ |
| **FP8 e4m3 / e5m2** | ✅ | ⚠️ | ✅ | ✅ |
...
| **Q4_0 (4.5 bit)** | ❌ | ✅ | ❌ | ❌ |
| **Q8_0 (8.5 bit)** | ❌ | ✅ | ❌ | ❌ |
| **TurboQuant 3-bit (TQ3)** | ✅ via Genesis | ✅ ([PR #21089](https://github.com/ggml-org/llama.cpp/pull/21089) WIP) | ⚠️ Same kernel bug as vLLM PR #40361 | ❌ |
| **2-bit KV** | ✅ ([PR #38479](https://github.com/vllm-project/vllm/pull/38479)) | ❌ | ❌ | ❌ |
| **CPU KV offload** | ✅ pluggable policies ([PR #37160](https://github.com/vllm-project/vllm/pull/37160)) | ⚠️ via mmap | ✅ Decode Radix Cache (v0.5.11) | N/A |
| **Disk KV offload** | ✅ FlexKV ([PR #34328](https://github.com/vllm-project/vllm/pull/34328)) + LMCache | ⚠️ | ⚠️ | ❌ |
...
| Method | vLLM | llama.cpp | SGLang | ktransformers |
|---|---|---|---|---|
| **MTP (Multi-Token Prediction)** | ✅ broad model coverage (Qwen3.x, Gemma4, MiniMax-M2, DeepSeek) | ✅ via [PR #22673](https://github.com/ggml-org/llama.cpp/pull/22673) (am17an) — community-built | ✅ Spec V2 default (overlap scheduling) | ⚠️ via SGLang front-end |
...
| **N-gram prompt-lookup** | ✅ GPU impl + async scheduler ([PR #29184](https://github.com/vllm-project/vllm/pull/29184)) | ✅ | ✅ | ❌ |
...
| **Layer-uniform expert offload to CPU** | ⚠️ generic `--cpu-offload-gb` (catastrophic for MoE) | ✅ `--n-cpu-moe N` / `-ot` regex | ⚠️ via kt-kernel | N/A |
| **Router-aware hot-expert caching** | ❌ | ❌ ([feature request #20757](https://github.com/ggml-org/llama.cpp/issues/20757) open) | ✅ via kt-kernel integration | ✅ Native (purpose-built) |
| **Disk-offload for cold experts** | ⚠️ via FlexKV / LMCache | ⚠️ via mmap | ⚠️ via kt-kernel | ⚠️ |
| **Shared-expert handling** | ✅ ([PR #35153](https://github.com/vllm-project/vllm/pull/35153) Oracle Flow) | ✅ | ✅ | ✅ |
...
| Feature | vLLM | llama.cpp | SGLang | ktransformers |
|---|---|---|---|---|
| **Tensor Parallel (TP)** | ✅ | ✅ `-sm tensor` | ✅ | ✅ |
| **Pipeline Parallel (PP)** | ✅ + cudagraphs ([PR #35162](https://github.com/vllm-project/vllm/pull/35162)) | ⚠️ Limited | ✅ | ✅ |
...
| **NVLink awareness** | ✅ | ⚠️ Generic NCCL | ✅ | ⚠️ |
| **Custom all-reduce** | ✅ (must disable on PCIe-only) | N/A | ✅ | N/A |
...
| Feature | vLLM | llama.cpp | SGLang | ktransformers |
|---|---|---|---|---|
| **Paged attention** | ✅ Original (Berkeley) | ⚠️ via continuous-batch slots | ✅ | ✅ via SGLang |
| **Prefix caching (in-VRAM)** | ✅ default-on | ✅ slot-based | ✅ **RadixAttention** (best-in-class) | ✅ via SGLang |
| **CPU-tier prefix cache (warm restart)** | ✅ via [LMCache](https://github.com/LMCache/LMCache) connector | ⚠️ via mmap | ✅ Decode Radix Cache (v0.5.11) | ⚠️ |
| **Disk-tier prefix cache (cold restart)** | ✅ LMCache + FlexKV | ⚠️ | ✅ Decode Radix Cache | ⚠️ |
| **Sparse attention (long ctx)** | ⚠️ HiSparse research | ⚠️ | ✅ HiSparse (v0.5.10) | ❌ |
| **Mamba/SSM hybrid handling** | ✅ Mamba state corruption fixes ([PR #37728](https://github.com/vllm-project/vllm/pull/37728)) | ⚠️ Limited | ✅ SSM-FA hybrid via NIXL | ⚠️ |
| **TMA / async copies (Hopper+)** | ✅ | ⚠️ | ✅ | ✅ |
| **Chunked prefill** | ✅ default | ⚠️ via parallel slots | ✅ | ✅ + Layerwise Prefill |
| **Continuous batching** | ✅ | ✅ via parallel slots | ✅ | ✅ via SGLang |

---

## Multimodal

| Modality | vLLM | llama.cpp | SGLang | ktransformers |
|---|---|---|---|---|
| **Vision (image)** | ✅ broad VLM coverage (Gemma4, Granite Vision, Hunyuan v3, ViT cudagraphs) | ✅ via `--mmproj` | ✅ optimized encoders | ⚠️ Limited |
| **Audio** | ✅ Nemotron / Qwen3-Omni | ✅ Granite Speech (b9045) | ⚠️ Limited | ❌ |
| **Video** | ⚠️ Frame-by-frame | ⚠️ | ✅ Diffusion (LTX-2, FLUX) | ❌ |
...
| Feature | vLLM | llama.cpp | SGLang | ktransformers |
|---|---|---|---|---|
| **OpenAI tool-call API compat** | ✅ | ✅ via `--jinja` | ✅ | ✅ via SGLang |
| **Custom tool parsers** | ✅ Per-model (Gemma4, Kimi-K2.5, GigaChat 3.1, qwen3_coder) | ⚠️ Generic | ✅ | ⚠️ |
| **Grammar (GBNF / EBNF)** | ✅ via outlines/lm-format-enforcer/xgrammar | ✅ Native GBNF | ✅ FSM-based | ⚠️ |
| **JSON schema mode** | ✅ | ✅ | ✅ Best-in-class FSM | ⚠️ |
| **Function calling enforcement** | ✅ tool-choice | ⚠️ Soft | ✅ Strict | ⚠️ |
| **Reasoning-channel separation** | ✅ qwen3 reasoning parser | ⚠️ | ✅ | ⚠️ |
...
| Model family | vLLM | llama.cpp | SGLang | ktransformers |
|---|---|---|---|---|
| **Qwen3.5 / Qwen3.6 (incl. 80B-A3B)** | ✅ | ✅ | ✅ Day-0 (Qwen3.6 v0.5.11) | ✅ |
| **Qwen3-Next family (DeltaNet hybrid)** | ✅ via Genesis patches | ✅ | ✅ | ⚠️ |
| **Gemma 4 / Gemma-4 31B** | ✅ + MTP ([PR #41745](https://github.com/vllm-project/vllm/pull/41745)) + DFlash ([PR #41703](https://github.com/vllm-project/vllm/pull/41703)) | ✅ via mmproj | ✅ Day-0 | ⚠️ |
...
| **MiniMax-M2 / M2.5 / M2.7 (incl. REAP)** | ⚠️ via custom code | ✅ via REAP'd GGUF | ✅ Day-0 M2.5 / M2.6 | ✅ Day-0 + native FP8 |
...
| **EXAONE-4.5 / Phi-4-reasoning-vision** | ✅ (v0.20) | ⚠️ | ⚠️ | ❌ |
...
| Feature | vLLM | llama.cpp | SGLang | ktransformers |
|---|---|---|---|---|
| **OpenAI-compatible HTTP** | ✅ | ✅ (`llama-server`) | ✅ | ✅ via SGLang |
| **Anthropic API compat** | ⚠️ | ❌ | ✅ Direct (v0.5.9) | ⚠️ |
| **Native Python SDK** | ✅ | ✅ | ✅ | ✅ |
| **gRPC** | ✅ ([PR #36169](https://github.com/vllm-project/vllm/pull/36169)) | ❌ | ⚠️ | ❌ |
| **Responses API streaming** | ✅ | ⚠️ | ✅ | ⚠️ |
| **Docker images (official)** | ✅ `vllm/vllm-openai` | ✅ `ghcr.io/ggml-org/llama.cpp` | ✅ `lmsysorg/sglang` | ❌ **No official Docker image** |
| **Pluggable backends** | ✅ | ✅ | ✅ kt-kernel pluggable | ✅ kt-kernel as backend |
...
| Issue | vLLM | llama.cpp | SGLang | ktransformers |
|---|---|---|---|---|
| **Marlin pad-sub-tile-n** (output-dim shards <64 on TP=2 W4A16) | 🟡 [PR #40361](https://github.com/vllm-project/vllm/pull/40361) open | N/A | 🟡 Same bug, same fix applies | N/A |
...
| **DFlash for Gemma 4** | 🟡 [PR #41703](https://github.com/vllm-project/vllm/pull/41703) Codex-rebased | ⚠️ Luce fork | ✅ Day-0 ROCm + CUDA | ❌ |
| **qwen3coder `<tool_call>`-in-prose silent SSE** | ⚫ Local sidecar (issue #72) | N/A | ⚫ Same root cause | N/A |
| **Per-token-head KV on hybrid pages** | 🟡 [PR #40391](https://github.com/vllm-project/vllm/pull/40391) | N/A | 🟡 | N/A |
| **Workspace lock strictness (vLLM 0.20)** | 🟢 [PR #39226](https://github.com/vllm-project/vllm/pull/39226) merged | N/A | N/A | N/A |
| **TurboQuant CPU + CUDA** | 🟢 ✅ via Genesis | 🟡 [PR #21089](https://github.com/ggml-org/llama.cpp/pull/21089) (CPU first) | 🟡 ⚠️ Same kernel bug as PR #40361 | ❌ |
...
This file ages quickly — all four projects ship multiple releases per month. **Refresh quarterly** (or whenever a major release substantively changes the comparison shape). When updating:
