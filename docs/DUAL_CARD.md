...
| **"Max accuracy" tier** (higher-fidelity weights + KV) | [`dual-max`](../models/qwen3.6-27b/vllm/compose/dual/fp8/mtp.yml) 🧪 (`vllm/qwen-27b-dual-max`) | **262K** | — (not yet TPS-benched) | tight @ TP=2 | **FP8** weights (e4m3, Marlin W8A16 on Ampere — memory win, no FP8 compute speedup) + **int8-PTH** KV (8-bit, higher fidelity than fast's fp8_e5m2). Boot/serve validated 2026-06-07 (KV pool 295K tok / 1.13× @262K, MTP active); quality A/B vs the fast tier pending. 🧪 Experimental. |
